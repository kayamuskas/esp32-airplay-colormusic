	...
	if (!strcmp(method, "OPTIONS")) {
		...
		if (body && (p = strcasestr(body, "volume")) != NULL) {
			...
			if (!dmap_parse(&settings, body, len)) {
                ...
                if ((p = kd_lookup(headers, "RTP-Info")) != NULL) sscanf(p, "%*[^=]=%" PRIu32, &timestamp);
            ...
            uint32_t timestamp = 0;
            if ((p = kd_lookup(headers, "RTP-Info")) != NULL) sscanf(p, "%*[^=]=%" PRIu32, &timestamp);
            LOG_INFO("[%p]: received JPEG image of %d bytes (ts:%d), ignoring", ctx, len, timestamp);
			// Artwork not used for LED strip, ignore
		} else {
...
static char *rsa_apply(unsigned char *input, int inlen, int *outlen, int mode)
{
    ...
    if (mbedtls_pk_parse_key(&pkctx, (unsigned char *)super_secret_key,
    ...
    size_t key_len = mbedtls_rsa_get_len(trsa);

    switch (mode) {
    case RSA_MODE_AUTH:
        mbedtls_rsa_set_padding(trsa, MBEDTLS_RSA_PKCS_V15, MBEDTLS_MD_NONE);
        outbuf = malloc(key_len);
        if (mbedtls_rsa_pkcs1_sign(trsa, mbedtls_ctr_drbg_random, &ctr_drbg,
                                   MBEDTLS_MD_NONE, (unsigned int)inlen, input, outbuf) == 0) {
            *outlen = (int)key_len;
        } else {
            free(outbuf);
            outbuf = NULL;
    ...
    case RSA_MODE_KEY:
        ...
        outbuf = malloc(key_len);
        if (mbedtls_rsa_pkcs1_decrypt(trsa, mbedtls_ctr_drbg_random, &ctr_drbg,
                                      &olen, input, outbuf, key_len) == 0) {
            *outlen = (int)olen;
        } else {
            free(outbuf);
            outbuf = NULL;
