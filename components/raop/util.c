...
in_addr_t get_localhost(char **name)
{
	...
	if (netif) {
		...
		if (name) {
			...
			*name = hostname ? strdup(hostname) : NULL;
		}
	}
...
bool http_parse(int sock, char *method, key_data_t *rkd, char **body, int *len)
{
	...
	while (read_line(sock, line, sizeof(line), timeout) > 0) {
		...
		LOG_SDEBUG("sock: %d, received %s", sock, line);
	...
	if (*len) {
		...
		if (content_len > max_body) {
			...
			while (remaining > 0) {
				...
				int bytes = recv(sock, tmp, chunk, 0);
				if (bytes <= 0) break;
				remaining -= bytes;
			}
			LOG_DEBUG("Discarded large body of %d bytes", content_len);
			*len = 0;
			*body = NULL;
			return true;
		}
		...
		*body = malloc(content_len + 1);
		if (!*body) {
			LOG_ERROR("content length alloc failed %d", content_len);
			return false;
		}
		while (size < content_len) {
			int bytes = recv(sock, *body + size, content_len - size, 0);
			if (bytes <= 0) break;
			size += bytes;
		}

		if (size < content_len) {
			LOG_ERROR("content length receive error %d %d", content_len, size);
			free(*body);
			*body = NULL;
			*len = 0;
			return false;
...
char *http_send(int sock, char *method, key_data_t *rkd)
{
	...
	if (!resp) resp = strdup("");
	data = malloc(strlen(method) + 2 + strlen(resp) + 2 + 1);
	if (!data) {
		NFREE(resp);
		return NULL;
	}
